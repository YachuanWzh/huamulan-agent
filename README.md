...
`huamulan-agent` 是一个面向 Agent 工程化的 LangGraph 工程：默认保持原有单 ReAct Agent 链路，同时新增可切换的 APM 多 Agent 模式，让主 Agent 可以编排 metrics、troubleshoot、patrol、audit 等子 Agent 分工分析。前端对应花木兰的人设与战备意象，后端保持安全 ReAct Agent、工具调用审批、渐进式 Skill 系统、长短期记忆、上下文压缩、RAG 知识库检索与评测闭环。
...
- **RAG 知识库**：单 Agent 模式可启用的检索增强生成。APM 运维知识 Markdown 文档经分块 + BGE-M3 embedding 存入 Qdrant，Agent 对话时自动检索相关知识片段注入 LLM 上下文。前端在 Assistant 回复顶部展示 📚 参考知识文档卡片，包含文档来源归属、相似度分数、章节名称和内容预览。
- **混合检索（Hybrid Retrieval）**：向量搜索 + BM25 关键词搜索（jieba 分词 + rank_bm25）+ RRF（Reciprocal Rank Fusion）融合 + LLM 相关性过滤（deepseek-v4-flash 元数据校验），提升中英混合术语（`trace_id`、`P99`、`CLS` 等）和精确关键词的召回质量。single-agent 与 multi-agent 模式均可启用，`KNOWLEDGE_HYBRID_ENABLED=true` 开启。
...
两条数据通道并行：**AlertManager Webhook** 处理 P0/P1 实时告警，**Kafka 批量消费** 处理 P2/P3 定时巡检。P0/P1 告警还会通过**飞书 Webhook** 推送卡片通知到群聊（可选启用）。
...
flowchart TB
    ...
    WEBHOOK -->|"P0 → 自动"| RCA
    WEBHOOK -->|"P0+P1"| SSE
    WEBHOOK -->|"P0+P1 告警到达<br/>send_alert()"| FEISHU
...
## 智能体工程平台（Agent Engineering Platform）

智能体工程工作台将 Trace、EvalRun、Regression、Replay 和 SBS 盲测整合为一个统一的复盘入口。完整报告见 [Agent工程平台建设报告.md](./Agent工程平台建设报告.md)。

```mermaid
flowchart LR
    subgraph 工程入口["飞行记录器 — 智能体工程工作台"]
        direction LR
        T["🔍 链路追踪"]
        R["📊 回归评测"]
        D["🔁 回放差异"]
    ...
    Agent["Agent 执行"] --> Logs["agent_execution_logs"]
    Logs --> Trace["Trace Hub<br/>父子 Span / 耗时 / Token / 脱敏"]
    Trace --> T

    Golden["Golden JSONL"] --> Eval["EvalRun Pipeline"]
    Eval --> Cases["逐 Case 结果 + 报告"]
    Cases --> Regression["Regression Comparator<br/>8 条可解释规则"]
    Regression --> R

    Checkpoint["LangGraph Checkpoints"] --> Diff["递归状态 Diff"]
    Diff --> Fork["Safe Fork Descriptor<br/>execute=false"]
    Diff --> D

    Harness["AgentHarness"] --> Parallel["并行真实运行<br/>两套模型 / Agent 配置"]
    Parallel --> Blind["盲化映射 A/B<br/>identity_map 不序列化"]
    Blind --> Review["不可变评审<br/>保存后锁定"]
    Review --> S
```
...
| 工具 | 回答的问题 | 核心能力 |
|------|-----------|----------|
| **链路追踪 (Trace)** | 一次 Agent 调用实际做了什么？慢在哪？错在哪？ | `trace_id` 统一标识 → 父子 Span 树 → 生命周期合并 → 敏感信息脱敏 → 时间脊柱可视化 |
| **回归评测 (Regression)** | 新版本相对基线是否退化？退在哪？ | EvalRun 持久化 → baseline vs candidate 比较 → 8 条规则（pass_to_fail / safety / forbidden_tool / latency / token）→ API + CLI 共享比较器 |
| **回放差异 (Replay)** | 两个 Checkpoint 的状态从哪里开始不同？ | 递归 diff → added / removed / changed → 类型变化检测 → Safe Fork（execute=false） |
| **SBS 评审 (Blinded SBS)** | 人更偏好哪套模型/Agent 的输出？ | 同一提示词并行真实运行 → 稳定随机盲化 A/B → tie / both_bad + 1-5 维度评分 → 首次保存锁定 → NDJSON 导出 → 删除管理 |

### 端到端数据流

```mermaid
flowchart TD
    User["用户请求 / Golden Case"] --> Harness["AgentHarness"]
    Harness --> TC["TraceContext<br/>trace_id / run_id / span_id"]
    TC --> Span["LLM / Tool / Multi-Agent Span"]
    Span --> DB[("agent_execution_logs<br/>+ evaluation_runs<br/>+ sbs_tasks")]
    DB --> API["Trace / Eval / Replay / SBS API"]
    API --> UI["EngineeringPanel.tsx<br/>四个标签页"]
    API --> CI["Regression CLI"]
    CI --> Exit["Exit Code: 0=通过 1=失败 2=无效"]
```
...
```mermaid
sequenceDiagram
    participant Eng as 工程工作台
    participant API as SBS API
    participant H as AgentHarness
    participant DB as SBS 存储

    Eng->>API: POST /api/sbs/tasks/run
    Note over API: 校验两套配置不相同
    API->>H: 并行调用 run_user_turn
    Note over H: 安全白名单自动批准<br/>危险命令审批阻塞
    H-->>API: 两份输出 + trace_id + 耗时
    API->>DB: 创建 SBS 任务
    API-->>Eng: SBSTask（含 provenance）

    Eng->>API: GET /api/sbs/tasks/{id}
    Note over API: 随机盲化 → Candidate A/B<br/>identity_map 不序列化
    API-->>Eng: BlindedSBSTask

    Eng->>API: POST reviews（winner + reason + scores）
    Note over API: 校验 both_bad 必须有理由<br/>维度评分 1-5
    API->>DB: 保存评审 + 锁定任务
    API-->>Eng: SBSReview（canonical_winner）

    Eng->>API: DELETE /api/sbs/tasks/{id}
    API->>DB: 删除任务及评审
```
...
| `KNOWLEDGE_RAG_ENABLED` | `false` | 是否启用单 Agent 模式 RAG 知识库检索 |
| `KNOWLEDGE_QDRANT_URL` | 可选，无默认值 | RAG 知识库 Qdrant HTTP API 地址，例如 `http://<qdrant-host>:6333`；启用 RAG 时必填 |
| `KNOWLEDGE_QDRANT_COLLECTION` | `apm_knowledge` | RAG 知识库 Qdrant collection 名称 |
| `KNOWLEDGE_QDRANT_API_KEY` | 可选，无默认值 | RAG 知识库 Qdrant API key |
| `KNOWLEDGE_RETRIEVAL_TOP_K` | `5` | 每次知识检索返回的 chunk 数量 |
| `KNOWLEDGE_HYBRID_ENABLED` | `false` | 是否启用混合检索（向量 + BM25 + RRF 融合 + LLM 相关性过滤）；启用后 single-agent 和 multi-agent 的 `user_vector_context` 均走混合检索管道 |
| `KNOWLEDGE_RELEVANCE_FILTER_ENABLED` | `true` | 混合检索是否启用 LLM 相关性过滤（依赖 `KNOWLEDGE_HYBRID_ENABLED`） |
| `KNOWLEDGE_RELEVANCE_FILTER_MODEL` | `deepseek-v4-flash` | 相关性过滤使用的轻量快速 LLM 模型 |
...
| `FEISHU_WEBHOOK_URL` | 可选，无默认值 | 飞书自定义机器人 Webhook 地址；配置后 P0/P1 告警自动推送卡片通知，RCA 完成后推送分析结果；不填则禁用飞书推送 |
| `FEISHU_WEBHOOK_SECRET` | 可选，无默认值 | 飞书机器人签名校验密钥；配置后 POST 附带 HMAC-SHA256 签名；不填跳过签名 |
...
├── api/          # FastAPI 服务器 + 数据模型 + 执行日志/摘要 API + 飞书推送通知
...
├── knowledge/    # RAG 知识库（文档分块、Qdrant 存储、向量检索、BM25 关键词搜索、混合检索（RRF 融合）、LLM 相关性过滤、来源归属、批量导入）
