...
- **RAG 知识库**：单 Agent 模式可启用的检索增强生成。APM 运维知识 Markdown 文档经分块 + BGE-M3 embedding 存入 Qdrant，Agent 对话时自动检索相关知识片段注入 LLM 上下文。前端在 Assistant 回复顶部展示 📚 参考知识文档卡片，包含文档来源归属、相似度分数、章节名称和内容预览。支持增量导入（`content_hash` 变更检测）
...
| RAG 知识库检索 | `backend/src/personal_assistant/knowledge/` | 单 Agent 模式可选启用的检索增强生成：Markdown 文档分块 → BGE-M3 embedding → Qdrant 向量搜索 → 中文来源归属 → LLM 上下文注入；前端展示 📚 参考知识文档卡片（文档名、相似度、章节、内容预览）；支持 `scripts/import_knowledge.py` 批量导入；详见[技术方案报告 §4.7](./技术方案报告.md#47-rag-知识库) |
...
├── knowledge/    # RAG 知识库（文档分块、Qdrant 存储、向量检索、来源归属、批量导入）
