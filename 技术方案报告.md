...
│       │       └── SKILL.md             # 审计标准操作流程
│       ├── knowledge/
│       │   ├── __init__.py              # 知识库模块导出 + build_knowledge_retriever 工厂
│       │   ├── chunker.py               # Markdown 文档分块器（H2 边界感知 + 上下文缓冲）
│       │   ├── models.py                # DocMeta / Chunk / SearchResult / SourceAttribution
│       │   ├── qdrant_store.py          # Qdrant CRUD 存储（upsert/search/delete/list）
...
- 当前仅单 Agent 模式（`agent_mode=single`）支持知识库检索，Multi-agent 模式使用独立的 `USER_VECTOR_RETRIEVAL_ENABLED` 用户向量检索
