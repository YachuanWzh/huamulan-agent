...
- **ClawEval 评测与评分卡**：提供 Golden Dataset 下拉选择、Quick 路由巡检、E2E Agent 实战测评、静态复杂度/行数/描述 token 评测、安全/工具/答案指标，以及评测结果落库；E2E 批量评测中普通工具自动授权，ToolGuard 危险工具自动拒绝
...
- 使用生产同款 `route_skill_names(...)`，覆盖 regex / triggers、语义召回、可选 rerank 和 LLM judge
- 不真正调用工具、不生成最终回答，速度极快，适合日常开发快速确认
- **产出指标**：
