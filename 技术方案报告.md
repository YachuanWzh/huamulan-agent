...
> **日期**: 2026-07-06  
> **状态**: 原型开发阶段  
> **更新内容**: 1. AlertManager Webhook 集成已实现，P0 自动 RCA + P1 手动触发 + P2/P3 Kafka 定时巡检；2. SSE 实时告警推送 + 前端 OTEL Alerts Panel 上线；3. 工具安全分级自动审批（`requires_rca_tool_approval`），危险操作阻塞为 `need_approve`；4. 新增飞书 Webhook 推送能力，P0/P1 告警触发与 RCA 完成时自动推送卡片通知  
...
flowchart LR
    ...
    subgraph 后端处理["langgraph-claw 后端"]
        ...
        RCA[_trigger_rca_background<br/>后台 RCA 线程]
    ...
    HANDLE -->|"P0: 自动触发 RCA"| RCA
    RCA -->|"RCA 完成/失败/阻塞<br/>send_rca_result()"| NOTIFIER
    NOTIFIER -->|"POST interactive card"| CARD1
    NOTIFIER -->|"POST interactive card"| CARD2
```
