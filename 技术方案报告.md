...
**智能体工程平台（Agent Engineering Platform）**：新增完整的智能体工程模块，将 Trace、EvalRun、Regression、Replay、SBS 盲测和 A/B Testing 自动对比整合为统一的可回溯证据体系。详见 [4.8 智能体工程平台](#48-智能体工程平台) 和 [Agent工程平台建设报告.md](./Agent工程平台建设报告.md)。
...
4. [核心模块设计](#4-核心模块设计)
   ...
   - [4.8 智能体工程平台](#48-智能体工程平台)
     ...
     - [4.8.6 A/B Testing —— Agent 变体自动对比](#486-ab-testing--agent-变体自动对比)
...
14. [CI/CD 与快检专项（v1.0.0）](#cicd-与快检专项v100)
15. [附录：演进路线](#附录演进路线)
...
flowchart TB
    ...
    subgraph HYBRID["Hybrid Retrieval (v1.0.0)"]
...
基础向量检索对于模糊语义查询效果良好，但对精确关键词（如 `trace_id`、`P99`、`Jaeger`、`CLS`）的召回不够敏感。v1.0.0 引入 `HybridRetriever`，将向量搜索与 BM25 关键词搜索通过 **RRF（Reciprocal Rank Fusion）** 融合，提升中英混合术语和精确关键词的召回质量。
...
#### 4.8.6 A/B Testing —— Agent 变体自动对比

A/B Testing 框架让你用**同一批 query 并行跑两组 Agent 变体**（不同模型/prompt/配置），机器自动收集多维度指标、做统计显著性检验、输出量化对比报告。和 SBS 盲评互补——A/B 先自动筛选出有真实差异的变体，SBS 再人工验证质量方向。

**一句话理解**：SBS 是"两个回答让人挑哪个好"，A/B Testing 是"同一批 query 各跑 N 遍，p-value 告诉你差异是真实改进还是随机波动"。

**架构概览**：

```mermaid
flowchart TB
    subgraph 输入["输入：JSONL Query 文件"]
        Q1["query: 分析服务延迟异常"]
        Q2["query: 查询过去1小时错误率"]
        Q3["query: ..."]
    end
    ...
    subgraph 变体["变体定义"]
        VA["Variant A<br/>model: deepseek-v4-pro<br/>agent_mode: multi"]
        VB["Variant B<br/>model: deepseek-v4-flash<br/>agent_mode: multi"]
    end

    subgraph 执行["ABTestRunner 并行执行"]
        direction LR
        subgraph A组["变体 A × N samples"]
            A1["run_user_turn<br/>thread_id=ab-A-0-xxx"]
            A2["run_user_turn<br/>thread_id=ab-A-1-xxx"]
        end
        subgraph B组["变体 B × N samples"]
            B1["run_user_turn<br/>thread_id=ab-B-0-xxx"]
            B2["run_user_turn<br/>thread_id=ab-B-1-xxx"]
        end
    end

    subgraph 采集["自动采集指标"]
        direction TB
        M1["success rate<br/>status=completed 比例"]
        M2["latency_ms<br/>端到端响应时间"]
        M3["judge_score<br/>LLM pairwise 评分"]
    end

    subgraph 统计["统计检验"]
        direction LR
        S1["Fisher's exact<br/>→ pass rate 差异"]
        S2["Mann-Whitney U<br/>→ latency 分布差异"]
        S3["Wilcoxon signed-rank<br/>→ judge score 差异"]
        S4["McNemar<br/>→ paired pass/fail"]
    end

    subgraph 输出["输出"]
        O1["ABTestReport<br/>JSON + Markdown 报告"]
    end
...
一个变体就是一个 JSON 配置块，描述你想测试的 Agent 是什么样：

```python
class ABVariant(BaseModel):
    name: str                          # "pro" / "flash" / "new-prompt"
...
每个 metric 不是简单比大小，而是**用假设检验判断差异是否真实**。通俗理解：p-value < 0.05 意味着"差异有 95% 以上概率不是碰巧出来的"。

| 维度 | 统计检验 | 通俗解释 |
|------|---------|---------|
| **pass_rate**（成功率） | Fisher's exact test + McNemar's test | 变体 A 的通过率 92%，变体 B 是 89%，降了 3 个点——但这个降幅可能只是运气差（跑一次而已）。Fisher 检验告诉你：如果跑无限次，A 和 B 的真实通过率到底有没有差别 |
| **latency_mean**（平均延迟） | Mann-Whitney U test | 不是比"平均数 A > 平均数 B"——而是比两组的**整体分布**。75% 的请求 A 比 B 快，但少数超慢请求拉高了 A 的平均值？Mann-Whitney 能发现这个 |
| **latency_p95**（P95 延迟） | descriptive（描述性） | 排序后取第 95 百分位，只看数字不看检验——因为 P95 天然就是单个值，没有分布可以检验 |
| **judge_score**（LLM 评分） | Wilcoxon signed-rank test | 同一道题 A 得分 3.8、B 得分 3.5，这个差异是所有题目都一致（配对检验），还是个别题目随机波动？Wilcoxon 告诉你答案 |
| **token_usage**（Token 消耗） | Paired t-test | 同一 query 跑 A 和 B 各一次，两者 token 数配对相减，t-test 判断差异均值是否显著偏离 0 |

**Judge LLM 配对评分**：

当启用 judge 时，框架用 LLM 对每个 query 的 A/B 两个回答做四维评分：

```
Query: "分析服务延迟异常"

Response A (pro):  "根因是 DB 连接池耗尽..." 
Response B (flash): "根因：连接池不够..."

Judge 输出 JSON:
{
  "score_a": 4.0,   ← correctness 5 + completeness 4 + clarity 4 + usefulness 3
  "score_b": 3.5,
  "winner": "A",
  "reason": "A 提供了具体的连接池参数和修复步骤，B 的回答正确但缺少操作细节"
}
```

Judge 复用项目已有的 `evaluation_judge_model`（`deepseek-v4-pro`，确保评判质量）。

**执行流程**（`ABTestRunner.run`）：
...
sequenceDiagram
    ...
    CLI->>Runner: run(queries, variant_a, variant_b, n_samples=3)

    loop 每个 query × 每个 sample
        Runner->>H: run_user_turn(A, query) ┐
        Runner->>H: run_user_turn(B, query) ├ asyncio.gather 并行
        H-->>Runner: ChatResponse(A)        ┘
        H-->>Runner: ChatResponse(B)
    end

    Runner->>Runner: 按 query 聚合<br/>success rate / latency / token

    opt Judge 启用
        loop 每个 query
            Runner->>Judge: pairwise(query, response_a, response_b)
            Judge-->>Runner: {score_a, score_b, winner}
        end
    end

    Runner->>Runner: 统计检验<br/>Fisher → pass rate 显著性<br/>Mann-Whitney → latency 显著性<br/>Wilcoxon → judge score 显著性

    Runner-->>CLI: ABTestReport
```

**输出的报告**就是之前演示的那样——表格化展示每个 metric 的 A 值、B 值、Δ%、p-value 和显著性标记（`***` p<0.001 / `**` p<0.01 / `*` p<0.05）。

**为什么自己实现统计检验**：

框架没有依赖 scipy，所有 5 种检验全部手写实现：

| 检验 | 核心实现 | 数学基础 |
|------|---------|---------|
| Fisher's exact | `fisher_exact_p()` | 超几何分布 + log binomial coefficient |
| Mann-Whitney U | `mann_whitney_u_p()` | 秩和 + 正态近似 + 连续性校正 |
| Paired t-test | `paired_t_test_p()` | Student's t-distribution CDF（通过不完全 beta 函数） |
| Wilcoxon signed-rank | `wilcoxon_signed_rank_p()` | 符号秩和 + 正态近似 |
| McNemar | `mcnemar_p()` | 小样本精确二项式 / 大样本卡方适配 |

底层数学原语（互补误差函数 `erfc` → 正态 CDF/分位数 / 不完全 beta 函数 `I_x(a,b)` → t 分布 CDF）全部从 `math` 标准库构建，零外部依赖。
...
# Prompt 对比：新旧 supervisor prompt
python scripts/run_ab_test.py \
  --queries queries.jsonl \
  --variant-a '{"name":"old"}' \
  --variant-b '{"name":"new","prompt_overrides":{"supervisor":"新 prompt 文本"}}' \
  --n-samples 2

# 快速对比（跳过 judge 节省成本）
python scripts/run_ab_test.py \
  --queries queries.jsonl \
  --variant-a '{"name":"A"}' --variant-b '{"name":"B"}' \
  --no-judge
```
...
| 操作 | 文件 | 说明 |
|------|------|------|
| 新建 | `skills/evaluation/ab_test.py` | `ABTestRunner` + `ABVariant` + 统计检验 + Judge + 报告格式化 |
| 新建 | `scripts/run_ab_test.py` | CLI 入口 |
| 新建 | `tests/test_ab_test.py` | 40 个单元测试（统计函数 + 数据模型 + Runner + Judge + 报告） |
| 修改 | `skills/evaluation/models.py` | 新增 `ABTestResult` 持久化模型 |
| 修改 | `skills/evaluation/__init__.py` | 导出 `ABTestResult` |
...
核心原则：**机器做统计 → 人做判断**。A/B Testing 告诉你"差了多少、是不是真的"，SBS 告诉你"差的方向对不对"。
...
| ✅ 智能体工程平台 | Trace Hub（全链路追踪 + 脱敏 + 拓扑）+ EvalRun（持久化评测 + 数据集哈希 + 配置快照）+ Regression Gate（8 条可解释规则 + API/CLI 共用比较器）+ Replay Debugger（递归状态 Diff + Safe Fork）+ SBS Blind Review（并行真实运行 + 盲化映射 + 不可变评审 + NDJSON）+ A/B Testing（自动统计检验 + LLM pairwise judge + 量化对比报告）+ EngineeringPanel 前端工作台 | 已完成 |
| ✅ CICD + 快检 | Woodpecker CI 全流程（后端 Ruff/Pytest + 前端 Oxlint/TypeScript/Vitest/Vite Build + Smoke Eval）+ ClawEval 快检集成 CI + Golden Dataset 回归验证 + 版本号正式切换 1.0.0 | 已完成 |
...
> 完整文档见 [CICD.md](./CICD.md)

v1.0.0 正式打通了 **Woodpecker CI + ClawEval 快检**全流程，形成"提交即检查、合并即验证"的自动化质量门禁体系。这是项目从原型验证向工程化交付的关键里程碑。
...
flowchart TB
    ...
    subgraph PIPELINE["Woodpecker CI Pipeline"]
        direction TB

        subgraph BE["后端质量门 (python:3.12-slim)"]
            LINT_BE["lint-backend<br/>Ruff 代码风格检查"]
            TEST_BE["test-backend<br/>Pytest 单元测试<br/>-m 'not integration'"]
            LINT_BE --> TEST_BE
        end

        subgraph FE["前端质量门 (node:22-alpine)"]
            LINT_FE["lint-frontend<br/>Oxlint"]
            TC_FE["typecheck-frontend<br/>tsc --noEmit"]
            TEST_FE["test-frontend<br/>Vitest"]
            BUILD_FE["build-frontend<br/>tsc -b && vite build"]
            LINT_FE --> TC_FE --> TEST_FE --> BUILD_FE
        end

        subgraph EVAL["Agent 行为门 (Smoke Eval)"]
            SMOKE["smoke-eval<br/>ClawEval 路由快检<br/>仅 backend 变更触发"]
        end
    end
    ...
    BE --> EVAL
    FE --> EVAL
    EVAL --> RESULT["✅ 全绿 → 合入 master<br/>❌ 失败 → 修复后重新推送"]
```
...
| 原则 | 说明 |
|------|------|
| **前后端并行** | 后端 Lint/Test 与前端 Lint/Typecheck/Test/Build 并行执行，互不阻塞 |
| **快检条件触发** | Smoke Eval 仅在 `backend/**` 变更时运行，纯前端或文档变更自动跳过，节省 LLM 调用成本 |
| **三层拦截** | 代码规范（Lint）→ 功能正确（Test）→ Agent 行为（快检），越靠后的检查成本越高、价值越大 |
| **与生产一致** | 快检使用与生产完全相同的三层漏斗路由逻辑（正则 → 语义 → LLM），确保测试结果可信 |
| **Secret 管理** | LLM API Key 通过 Woodpecker Secret 注入，不硬编码 |

### 快检：ClawEval Smoke Eval

快检是整个 CI/CD 中最具"AI 项目特色"的环节——它不是在测代码逻辑，而是在验证 **Agent 还能不能正确理解用户意图并选中技能**。

```mermaid
flowchart LR
    GOLDEN["Golden Dataset<br/>otel_routing.jsonl<br/>约 40 条标准问法"] --> ROUTE

    subgraph ROUTE["三层漏斗路由"]
        T1["Tier 1: 正则匹配<br/>SKILL.md triggers"]
        T2["Tier 2: 语义检索<br/>BGE-M3 + Qdrant"]
        T3["Tier 3: LLM 判定<br/>DeepSeek API"]
        T1 --> T2 --> T3
    end

    ROUTE --> COMPARE["对比期望 vs 实际"]
    COMPARE --> REPORT["eval-report.md<br/>路由准确率 / 误触发率<br/>静态复杂度评分"]
```
...
| 指标 | 含义 | 目标 |
|------|------|------|
| Selection Accuracy | 正例中 Skill 选择与期望完全一致的比例 | > 90% |
| False Positive Rate | 负例中误触发 Skill 的比例 | < 5% |
| Description Tokens | Skill 描述长度估算 | 合理范围 |
| Complexity | Python 圈复杂度 | 越低越好 |

### CI 流水线配置

配置位于 `.woodpecker.yml`，关键要点：
...
variables:
  PYTHONUNBUFFERED: "1"  # CI 日志实时输出

steps:
  lint-backend:    # Ruff 检查 src/ 和 tests/
  test-backend:    # Pytest -m "not integration" --timeout=120
  lint-frontend:   # Oxlint
  typecheck-frontend: # tsc --noEmit
  test-frontend:   # Vitest --maxWorkers=1
  build-frontend:  # Vite 生产构建
  smoke-eval:      # ClawEval 快检（条件触发）
```
...
smoke-eval:
  when:
    path:
      include: ["backend/**", ".woodpecker.yml"]  # 后端或 CI 配置变更
      exclude: ["*.md", "docs/**"]                # 文档变更跳过
```
...
```mermaid
flowchart LR
    DEV["💻 本地开发<br/>编写代码 + 测试"] --> LINT["📏 本地 Lint + Test<br/>ruff/oxlint/pytest/vitest"]
    LINT --> SMOKE["⚡ 本地快检<br/>改路由/Skill 时必跑"]
    SMOKE --> PUSH["⬆️ git push / PR"]
    PUSH --> CI["🔄 CI Pipeline<br/>7 阶段自动执行"]
...
| 阶段 | 规划 | 说明 |
|------|------|------|
| 短期 | E2E 快检集成 CI | 将实战测评（完整 Agent 链路）纳入 CI，覆盖工具调用和安全拦截 |
| 短期 | 前端快检 CI | 将前端组件测试快检加入 smoke-eval 阶段 |
| 中期 | 评测趋势看板 | 在 CI 中记录每次快检的历史数据，追踪 Skill 路由质量变化趋势 |
| 中期 | 自动修复建议 | 快检失败时自动分析原因，生成修复建议 |
| 长期 | 部署流水线 | Docker 构建 → 镜像推送 → 自动部署，打通完整 CI/CD 环
