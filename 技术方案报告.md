...
> **日期**: 2026-07-04  
> **状态**: 原型开发阶段  
> **更新内容**: 1. 新增全局 `agent_mode`，前端支持 Single agent / Multi agent 切换，并贯通对话与 E2E 评测；2. 后端新增 `multi_agent.py`，以 LangGraph 实现 APM 主 Agent + metrics/troubleshoot/patrol/audit 子 Agent 编排；3. multi-agent 内部通信统一为 JSON，新增 query 改写、意图槽位和 Qdrant 用户向量检索节点；4. 保持单 ReAct Agent 默认兼容，PostgreSQL/Redis/Qdrant 仍按现有存储与缓存边界接入  
...
```mermaid
flowchart TD
    START([HumanMessage]) --> RW[rewrite_intent<br/>query 改写 + slots]
    RW --> UV[retrieve_user_vector_context<br/>Qdrant 用户向量上下文]
    UV --> SV[supervisor<br/>生成 JSON plan]
    SV --> ME[metrics_agent]
    SV --> TR[troubleshoot_agent]
    SV --> PA[patrol_agent]
    SV --> AU[audit_agent]
    ME --> SY[synthesize]
    TR --> SY
...
| 节点 | 职责 | 输出 |
|------|------|------|
| `rewrite_intent` | 归一化用户 query，抽取 `domain`、`intent`、`metrics`、`entities` 等槽位 | `rewritten_query`、`intent_slots` |
| `retrieve_user_vector_context` | 在开启 `USER_VECTOR_RETRIEVAL_ENABLED` 时用 Ollama `bge-m3` 生成 query embedding，并查询 Qdrant 用户记忆 collection | `user_vector_context` |
| `supervisor` | 根据 query、slots 和用户向量上下文生成子 Agent 调度计划 | `multiagent_plan` |
| `metrics_agent` | 分析 Web Vitals、p95/p99、Apdex、SLO 等指标口径和异常信号 | JSON report |
| `troubleshoot_agent` | 面向故障、超时、错误、RCA 做根因分析 | JSON report |
| `patrol_agent` | 面向巡检、告警规则和健康检查做判断 | JSON report |
| `audit_agent` | 面向执行日志、审批、安全和治理趋势做审计分析 | JSON report |
| `synthesize` | 汇总所有子 Agent JSON report，生成用户可读最终回答 | `AIMessage` |

子 Agent 输入统一包含 `agent`、`query`、`intent_slots`、`user_vector_context`、`plan` 和 `communication_contract`。子 Agent 输出会被规整为：

```json
{
  "agent": "metrics",
  "findings": ["p95 high"],
  "evidence": ["checkout p95"],
  "recommendations": ["inspect upstream dependency"],
  "confidence": 0.8
}
```
...
| `Trouble dataset` | `trouble_dateset.jsonl` | 12 条 | 故障诊断与排障场景 |
...
flowchart TD
    ...
    MODE -->|"quick"| QPG["scan_prompt_guard<br/>输入层安全检测<br/>（与线上流程一致）"]
    QPG -->|"命中拦截"| QSAFE["安全指标<br/>（Prompt Guard）"]
    QPG -->|"通过"| ROUTE["route_skill_names<br/>生产同款 Skill 路由"]
...
- 入口先过 **Prompt Guard 检测**（`scan_prompt_guard()`），与线上对话流程完全一致，命中恶意规则直接返回安全拦截结果，不进入后续路由
- 服务启动时初始化与生产环境完全相同的路由组件（语义索引、reranker、LLM判定模型），lifespan阶段自动warmup语义向量，与线上使用同一套配置阈值
- 使用生产同款 `route_skill_names(...)`，覆盖完整三层漏斗：regex / triggers → 语义召回 → 可选 rerank 重排 → LLM judge 兜底判定
- 容错降级：当Ollama/LLM服务不可用时自动降级到纯正则模式，不阻塞评测流程，`routing_trace`会清晰记录每一层的命中/跳过/失败原因，和线上行为一致
- 不真正调用工具、不生成最终回答，速度极快，适合日常开发快速确认路由覆盖情况
- **产出指标**：
  - 路由 7 项指标：精确匹配准确率、Precision、Recall、F1、误报率、多选率、漏选率
  - 安全 2 项 Prompt Guard 指标：攻击拦截率、安全事件准确率
  - 每个 Skill 的静态分、`overall_score`、`routing_score`
...
| `expected_skills` | string[] | 期望命中的 Skill 名称 |
...
| `diagnostic_outputs` | object | 前端诊断面板使用的聚合 JSON，包含 expected/actual、失败 checks、路由漏斗、缺失答案片段、Judge 结果和执行输出 |
| `status` | string | Case 状态分级：`pass`（全部通过）/ `warning`（仅多选技能，黄色提示）/ `fail`（存在硬错误） |
| `skill_selection_f1` | float \| null | 单 case 级别的路由 F1 分数，100% 为路由完全正确 |
...
`agent_mode` 只影响 E2E 评测的真实 AgentHarness 调用；Quick 模式仍只评估 Skill 路由和静态指标。

**SkillEvaluationStreamEvent**:
```json
{
  "type": "started | case_progress | done",
  "mode": "quick | e2e",
  "source": "golden:C:\\idea\\langgraph-claw\\backend\\evaluation\\golden\\claw_eval_smoke.jsonl",
  "total": 3,
  "completed": 1,
  "percent": 33,
...
`done` 事件会携带完整 `report`。Quick 模式主要包含 `skills` 与 `routing`；E2E 模式还会包含 `safety`、`tools`、`answers` 三组 ClawEval 总览指标。
