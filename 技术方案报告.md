...
> **日期**: 2026-07-03  
> **状态**: 原型开发阶段  
> **更新内容**: 1. Quick快检模式支持完整三层路由漏斗（正则→语义检索→LLM判定），与生产环境路由逻辑完全一致，服务不可用时自动降级；2. 修复patrol技能正则覆盖不全问题，新增"巡检"、"告警阈值"等核心词匹配，支持无时间窗口的中文阈值场景  
...
- **ClawEval 评测与评分卡**：提供 Golden Dataset 下拉选择、Quick 路由巡检、E2E Agent 实战测评、静态复杂度/行数/描述 token 评测、安全/工具/答案指标，以及评测结果落库；E2E 批量评测中普通工具自动授权，ToolGuard 危险工具自动拒绝；失败/告警 case 会把最终回答、执行输出、Judge 证据和 `suspected_node` 返回前端，辅助判断问题更可能出在 prompt、skill、tool 或安全护栏
...
| 选项 | 文件 | 用途 |
|------|------|------|
| `ClawEval smoke` | `claw_eval_smoke.jsonl` | 最小冒烟集，确认评测链路可用 |
| `Golden dataset` | `golden_dataset.jsonl` | 常规 Skill 路由与评分集 |
| `E2E dataset` | `e2e_dateset.jsonl` | 端到端 Agent 行为评测集 |
...
- **产出指标**：路由 7 项 + 安全 4 项 + 工具 6 项 + 回答 2 项 + 幻觉 5 项，全维度覆盖
- 在 `done` SSE 中返回完整 `report`，前端展示 ClawEval 总览。

**E2E 模式**

- 使用真实 `AgentHarness.run_user_turn(...)` 执行 case。
- 支持 `turns` 多轮输入；没有 `turns` 时使用 `query`。
- 遇到工具审批时自动处理：
  - `scan_tool_guard(tool_name, args)` 命中危险操作：自动拒绝。
  - 未命中 ToolGuard：自动通过。
...
路由漏斗用于回答“为什么漏选 Skill”。`regex` 节点说明正则和 trigger 是否命中；`semantic` 节点展示向量召回候选、分数和阈值；启用 rerank 时会追加 `rerank` 节点；只有存在低分语义候选且配置了 LLM 路由 judge 时，才会进入 `llm_judge` 节点。常见状态包括 `selected`、`missed`、`skipped`、`below_threshold`、`rejected`、`failed`。

前端在失败/告警 case 中展示 `Diagnostic output` 面板，包含 `Suspected node` 标签、`Routing funnel` 节点、`Final answer`、`Missing expected content`、`Judge` JSON 和 `Execution outputs` JSON。这样一条失败 case 不再只显示“Final answer missed expected content”，而是能看到实际回答、路由卡在哪层、Judge 证据、工具或 LLM 输出，以及应该优先查 prompt、skill 还是 tool。
...
- `RoutingMetrics`：路由评测结果，包括命中率和误触发率。
- `StaticSkillMetrics`：静态指标，包括描述 token、行数、复杂度、工具数。
- `RuntimeSkillMetrics`：动态指标，包括调用数、成功率、重试率、延迟、token 消耗。
- `SafetyEvaluationMetrics`：安全拦截、危险工具和泄露类指标。
- `ToolEvaluationMetrics`：工具选择、参数一致性和禁止工具违规指标。
- `AnswerEvaluationMetrics`：答案包含率和 forbidden 内容违规率。
- `CaseEvaluationDetail`：单 case 诊断详情，包含 deterministic checks、`diagnosis`、`final_answer`、`log_summary`、`judge`、`suspected_node`、`routing_trace` 和 `diagnostic_outputs`。
- `SkillEvaluationReport`：最终评分卡，既可导出 JSON / Markdown，也可落库供前端展示。
...
- 支持“快速巡检”和“实战测评”两个按钮。快速巡检用于路由和静态评分，实战测评用于完整 Agent E2E。
...
flowchart LR
    ...
    RCA --> Panel
    User["用户排障/巡检请求"] --> Router["Skill 路由"]
...
8. 前端 Performance 面板展示汇总结果；对话侧可以继续调用 troubleshoot、patrol、runbook 或 audit-sop Skill 做进一步分析。
