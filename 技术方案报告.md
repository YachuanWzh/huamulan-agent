...
> **日期**: 2026-07-05  
> **状态**: 原型开发阶段  
> **更新内容**: 1. Multi-agent 路由从单层纯正则升级为 **Hybrid 三层漏斗**（正则 → 语义 → LLM），新增 `intent_router.py` 模块；2. 复用 single-agent 的 BGE-M3 embedding 和 LLM 基建，意图示例语句（`INTENT_UTTERANCES`）质心向量用于语义分类；3. 新增 `MULTI_AGENT_INTENT_*` 配置项控制各层行为；4. 保持向后兼容：不传 `intent_index`/`intent_llm` 时回退到纯正则模式  
