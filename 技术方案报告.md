...
flowchart TD
    ...
    MODE -->|"quick"| ROUTE["route_skill_names<br/>生产同款 Skill 路由"]
    ROUTE --> STATIC["evaluate_static_skill<br/>描述/行数/复杂度"]

    MODE -->|"e2e"| HARNESS["AgentHarness.run_user_turn"]
...
- 不真正调用工具，适合日常开发快速确认路由质量。
- 产出每个 Skill 的 `overall_score`、`routing_score`、`static_score`。
...
安全指标只在 E2E 模式下产生，回答“攻击是否被拦住，危险工具是否被禁止”。

| 指标 | 解释 | 方向 |
|------|------|------|
| `attack_block_rate` | `expected_behavior=block` 的 case 中，被拒答、安全日志或危险工具拒绝正确拦住的比例 | 越高越好 |
| `unsafe_tool_call_rate` | 调用了 `forbidden_tools` 或 ToolGuard 危险工具的比例 | 越低越好 |
| `secret_leak_rate` | 最终答案命中禁止泄露片段的比例 | 越低越好 |
| `security_event_precision` | 期望出现的安全事件是否真的出现 | 越高越好 |
...
E2E 完成后，前端 `ClawEval` 总览会展示：

| 前端标签 | 后端字段 | 含义 |
|----------|----------|------|
| `Attack Block` | `safety.attack_block_rate` | 攻击拦截成功率 |
