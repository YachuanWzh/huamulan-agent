...
| 数据库 | PostgreSQL + psycopg | ≥3.2 | Checkpoint 持久化 + 审计日志 |
...
flowchart TB
    ...
    subgraph Backend["Backend — FastAPI + Uvicorn (ASGI)"]
        ...
        subgraph INFRA["Infrastructure"]
            ...
            PM[PostgresMemory<br/>Checkpoint + Audit]
            HK[AgentHookManager<br/>Hook 生命周期]
            ST[ScriptTool<br/>声明式脚本工具]
            BT[BasicTools<br/>Shell / File / List / Search]
        end
    end
...
│       ├── memory/
│       │   └── postgres.py              # PostgreSQL Checkpoint + 审计
│       ├── skills/
...
│       └── tools/
│           ├── __init__.py
│           └── basic.py                 # 基础工具 (shell/file/list/search)
├── frontend/
│   ├── package.json
...
Agent 的核心是一个 4 节点的 LangGraph `StateGraph`：
...
flowchart TD
    ...
    RS --> AG[agent<br/>LLM 推理 + 工具选择]
    AG --> AP2[approval<br/>审批门检查]
...
flowchart TD
    ...
    WAIT["wait (END)<br/>前端显示审批卡片<br/>等待用户 Approve / Deny"]
    WAIT -->|"用户审批后<br/>resume_after_approval_stream()"| RS2["_entry_route<br/>直入 approval"]

    subgraph L5["Layer 5: 工具执行"]
        TL["tools<br/>agent.py:49 + harness.py:405<br/>1. Tool Guard 安全扫描<br/>2. Tool Middlewares<br/>   频率 / 数量 / 循环<br/>3. ToolNode 执行"]
    end
...
class PostgresMemory:
    ...
    async def start(self)  → 打开连接池 + 初始化 checkpoint 表 + 创建审计表
