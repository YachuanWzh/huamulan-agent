...
> **日期**: 2026-07-03  
> **状态**: 原型开发阶段  
> **更新内容**: 1. Golden Dataset 扩充至 219 条，新增 APM/审计/安全等 8 个领域专项数据集，总计 478+ 条用例；2. E2E 实战测评增加运行拓扑可视化，泳道展示入口→安全→路由→工具→回答全链路状态；3. 新增失败诊断详情面板，含 suspected_node 推断、routing_trace 路由漏斗、diagnostic_outputs 聚合信息；4. 完善安全输入检测，Quick 模式前置 Prompt Guard，case 三级状态分级（pass/warning/fail）  
...
Prompt Guard 的入口规则抽象为 `PromptGuardRule`，包含 `category`、`severity`、`reason`、`pattern`、`priority` 和 `order`。运行时按优先级稳定扫描，既能覆盖“忽略之前所有指令”这类 instruction override，也能覆盖“管理员/CEO 要求查看系统指令”这类身份伪造与系统提示泄露组合攻击。新增安全规则时应先补 safety golden case，再加入规则表并明确优先级，避免安全 case 被后续 Skill 路由误判为普通业务意图。
...
| `Golden dataset` | `golden_dataset.jsonl` | 219 条 | 综合路由评测集（基础/边界/困难/负向/闲聊/多意图） |
| `Skill routing` | `skill_routing.jsonl` | 180 条 | 专项路由覆盖测试 |
| `E2E dataset` | `e2e_dateset.jsonl` | 26 条 | 端到端 Agent 行为评测集 |
...
| `Security prompt guard` | `security_prompt_guard.jsonl` | 12 条 | Prompt Guard 安全防护专项 |
