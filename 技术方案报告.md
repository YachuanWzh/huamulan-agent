...
3. **LLM judge 层**：当 top 候选低于 `SKILL_ROUTING_SIMILARITY_THRESHOLD` 但仍有召回结果时，构造 `{"userInput": "...", "relatedFind": [...]}` 交给 LLM，并用本地 `LLMSkillRouteDecision` schema 校验 JSON 输出。若结构不合法，会把 `previousError` 与 `previousOutput` 传回 LLM 重试。该层可通过 `SKILL_ROUTING_LLM_MODEL` 使用独立模型（例如 `deepseek-v4-flash`），未配置时沿用主 `LLM_MODEL`。

语义路由失败（Ollama/Qdrant 不可用、collection 未准备好、网络异常等）不会中断对话；路由会安全降级为未选中 Skill，System Prompt 不再暴露任何 Skill 元数据。只有漏斗最终选中的 Skill 会进入本轮 System Prompt 和工具过滤范围，以降低 Skill 数量变多或描述相近时的误召回风险。
...
语义路由是可选能力，由 `SKILL_ROUTING_SEMANTIC_ENABLED` 控制。开启后，系统使用 Ollama `bge-m3` 生成 embedding，并通过统一的 `SkillVectorIndex` 接口执行 Skill 召回：
...
| `SKILL_ROUTING_SEMANTIC_ENABLED` | `false` | 是否启用 regex → embedding 召回 → LLM judge 的三层 Skill 路由漏斗 |
| `SKILL_ROUTING_EMBEDDING_MODEL` | `bge-m3` | Ollama embedding 模型名称 |
| `SKILL_ROUTING_OLLAMA_BASE_URL` | `http://localhost:11434` | Ollama 服务地址；局域网部署时填写 bge-m3 服务机器地址 |
...
| `SKILL_ROUTING_TOP_K` | `3` | 语义召回候选数量，低于阈值时交给 LLM judge |
