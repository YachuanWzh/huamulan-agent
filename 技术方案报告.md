...
Skill 评测模块位于 `personal_assistant.skills.evaluation`。它把一个 Skill 的质量拆成四个问题：

- **能不能被选对**：用户问题进来时，路由器是否选中正确 Skill。
- **能不能跑稳**：被调用后是否成功、是否频繁重试。
- **是否好维护**：描述是否清晰精炼，代码是否过长或过复杂。
- **是否真的常用**：运行日志里是否有足够调用量。
...
离线评测会调用生产同款 `route_skill_names(...)`，因此能真实覆盖 regex / triggers、语义召回、可选 rerank 和 LLM judge 的组合行为。前端 Skill Evaluation 页面也可以输入 Golden Dataset 路径并触发同一套评测。
