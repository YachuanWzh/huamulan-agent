...
> **日期**: 2026-07-05  
> **状态**: 原型开发阶段  
> **更新内容**: 1. 接入 OpenTelemetry Demo 远程遥测数据，新增 `otel-query` Skill 支持 Jaeger 追踪查询与 Prometheus 指标查询；2. `apm.py` 新增 Jaeger trace → `FrontendRumEvent`/`ExecutionLog` 转换器与 Prometheus metric → `ExecutionLog` 转换器；3. 完善 OTEL 数据集成技术方案（数据产生 / 数据获取→APM分析 / 未来推送自动分析规划）  
...
13. [OTEL 远程遥测数据集成](#otel-远程遥测数据集成)
    ...
    - [未来规划：遥测数据推送与自动分析](#4-未来规划遥测数据推送与自动分析)
14. [附录：演进路线](#附录演进路线)
...
当前实现依赖用户通过 Agent 对话**主动触发**遥测数据查询。未来将演进为**推送驱动**的自动化分析模式。

#### 4.1 短期目标（当前架构内）

| 规划 | 说明 | 依赖 |
|------|------|------|
| **SLO 基线配置** | 在 `apm-metrics` Skill 中为关键服务配置 P95 延迟、错误率、吞吐量的 SLO 基线，使 metrics_agent 的异常判定有明确参照 | 无额外依赖 |
| **Golden Dataset 扩展** | 新增 `otel_single_agent.jsonl`、`otel_multi_agent.jsonl`、`otel_routing.jsonl` 评测数据集，覆盖 OTEL 遥测查询的典型场景，用于回归验证分析质量 | 已完成数据文件准备 |
| **PromQL 模板库** | 在 `apm.py` 中预置常用 PromQL 模板（P95/P99/Apdex/Error Rate/SLO Compliance），降低 Agent 编写 PromQL 的错误率 | 无额外依赖 |
| **Trace 可视化** | 在前端 WorkspacePanel 增加 Trace 瀑布图视图，将 Jaeger span 层级关系渲染为可视化时间轴 | 前端开发 |
| **定时巡检集成** | 将 `patrol_agent` 与系统 Cron 集成，支持"每 30 分钟自动巡检所有服务并报告异常" | 后端 Cron 调度 |

#### 4.2 中期目标（推送机制）

| 规划 | 说明 | 关键设计 |
|------|------|----------|
| **Prometheus AlertManager 集成** | 当 Prometheus 触发告警（如 P95 > 500ms）时，通过 Webhook 推送到 langgraph-claw，自动触发 troubleshoot_agent 进行 RCA | Webhook 接收端点 + Alert 消息解析 |
| **OTel Collector 直推** | 在远端 OTel Collector 中配置 `otlp_http` exporter，将精选 trace/metric 推送到 langgraph-claw 的分析端点，实现准实时分析 | 新增 OTel Collector exporter 配置 + langgraph-claw 接收端点 |
| **异常自动分级响应** | 根据异常严重度自动升级：L1（P95 轻微超标）→ 记录到巡检报告；L2（错误率突增）→ 自动发起排障；L3（服务不可用）→ 通知 + 排障 + Runbook 建议 | 异常分级规则引擎 |

#### 4.3 长期愿景

```mermaid
flowchart LR
    subgraph Push["推送驱动的自动分析"]
        ALERT["Prometheus Alert"] --> WEBHOOK["AlertManager Webhook"]
        WEBHOOK --> CLAW["langgraph-claw<br/>自动分析入口"]
    end

    subgraph AutoAnalysis["langgraph-claw 自动分析流水线"]
        CLAW --> PARSE["解析告警上下文<br/>service / metric / threshold"]
        PARSE --> FETCH["自动查询关联遥测<br/>Jaeger traces + Prometheus metrics"]
        FETCH --> RCA["apm.py 根因分析"]
        RCA --> REPORT["生成结构化诊断报告"]
    end

    subgraph Action["行动与通知"]
        REPORT --> NOTIFY["通知相关人员<br/>（邮件 / 消息 / 工单）"]
        REPORT --> RUNBOOK["自动输出 Runbook<br/>修复步骤"]
        REPORT --> DASHBOARD["更新巡检面板<br/>遗留待处理项"]
    end

    Push --> AutoAnalysis --> Action
```

**核心能力演进方向**：

1. **零人工介入的智能排障**：从"用户问 → Agent 查"演进为"系统检测 → Agent 自动分析 → 推送结论"
2. **跨服务依赖拓扑感知**：利用 Jaeger trace 中的 parent-child span 关系，自动构建服务依赖图，在根因分析时沿调用链向上追溯
3. **历史基线自学习**：基于 Prometheus 历史数据学习每个服务的季节性和趋势模式，减少静态阈值的误报
4. **修复建议闭环**：将排障结论与实际的修复操作关联，形成"诊断 → 修复 → 验证"的反馈闭环
