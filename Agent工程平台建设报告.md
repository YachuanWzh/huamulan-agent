...
两套配置不能完全相同，避免为无意义的重复执行消耗 Token。两套配置不能完全相同，避免为无意义的重复执行消耗 Token。SBS 复用项目现有评测安全策略：通过 `_TOOL_PATTERNS` 白名单匹配的只读查询、日志查看、安全 bash 命令自动执行；命中危险命令（磁盘格式化、反弹 Shell、提权等）或写操作时仍通过审批阻塞，确保并行运行不绕过安全边界。运行失败或没有输出时同样不创建残缺任务。手工粘贴已有输出仍然保留，但只作为"导入已有输出（高级）"的兼容入口，默认折叠。“导入已有输出（高级）”的兼容入口。
...
| DELETE | `/api/sbs/tasks/{task_id}` | 删除指定评审记录（含候选输出与评审结果） |
...
- 加载评审队列，每条记录支持删除（含确认对话框）；
- 只展示盲化后的 A/B；
- 选择 winner、填写 reviewer 和 reason；
- both_bad 没有理由时按钮不可用。

视觉设计沿用项目既有的花木兰暖色工作台，全模块已中文本地化（链路追踪/回归评测/回放差异/SBS 评审），SBS 配置面板采用左右并排布局，模型下拉从后端实时获取。支持键盘 focus、移动端单列布局和 `prefers-reduced-motion`。
