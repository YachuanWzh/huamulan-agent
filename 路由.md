...
- **Multi-agent 模式**：Hybrid 三层漏斗（正则 → 语义 → LLM），将用户输入分类为**粗粒度意图**，再并行分发给 4 个 APM 子 Agent。复用 Single-agent 的 BGE-M3 embedding 和 LLM 基建
...
flowchart TD
    ...
    M_GRAPH --> M_ROUTE["🎯 Intent Router<br/>三层漏斗：正则 → 语义 → LLM<br/>→ intent + slots + confidence"]
    ...
    style M_ROUTE fill:#4a9,stroke:#287,color:#fff
...
核心入口：`route_intent_with_trace()`（`intent_router.py`），由 `rewrite_intent` 节点（`multi_agent.py`）调用。

> **2026-07-05 更新**：Multi-agent 路由已从单层纯正则升级为 **Hybrid 三层漏斗**（正则 → 语义 → LLM），与 Single-agent 模式共享同一套 embedding 基础设施（`OllamaBgeM3EmbeddingProvider`）。详见下方 2.2 节。

Multi-agent 模式只服务 APM 可观测性场景。三层漏斗中的每一层：
- **Tier 0（正则 + 置信度）**：保留 `rewrite_query_and_slots()` 提取 metrics/entities，新增 `_regex_intent_with_confidence()` 做带置信度的意图分类（≥0.80 短路）
- **Tier 1（语义检索）**：`IntentEmbeddingIndex` 用 BGE-M3 将意图示例语句（`INTENT_UTTERANCES`）embedding 后均值池化为意图质心，查询时与质心做余弦相似度匹配
- **Tier 2（LLM 分类器）**：`IntentDecision` 结构化输出，支持 primary + secondary intents + confidence + reason

意图识别结果驱动后续 4 个子 Agent 的并行调度。
...
flowchart TD
    ...
    PG -->|拦截| BLOCK["❌ 返回拒绝"]
    PG -->|放行| RW["<b>rewrite_intent</b><br/>🎯 Hybrid 三层漏斗<br/>正则 → 语义 → LLM<br/>→ intent_slots + trace"]
...
### 2.2 rewrite_intent（意图识别核心 — 已升级为三层漏斗）

> **实现状态**：三层漏斗已完整实现，代码在 `intent_router.py`，由 `multi_agent.py` 的 `rewrite_intent` 节点编排调用。
> 可通过 `MULTI_AGENT_INTENT_*` 环境变量控制各层行为。旧 `rewrite_query_and_slots()` 仍保留用于 metrics/entities 提取。

`rewrite_intent` 节点执行流程（`multi_agent.py`）：

```python
async def rewrite_intent(state, config):
    query = _last_human_text(state)
    legacy = rewrite_query_and_slots(query)  # 保留：提取 metrics/entities

    # 三层漏斗（当 intent_index 或 intent_llm 可用时）
    routing = await route_intent_with_trace(
        query,
        intent_index=intent_index,    # Tier 1: IntentEmbeddingIndex
        llm=intent_llm,               # Tier 2: LLM 分类器
        existing_slots=legacy["slots"],  # 复用正则提取的指标/实体
    )
    return {
        "rewritten_query": legacy["rewritten_query"],
        "intent_slots": routing.intent_slots.to_dict(),  # 兼容旧格式
    }
```

`rewrite_query_and_slots()`（`multi_agent.py:22-54`）完成**Query 规范化 + 指标提取 + 实体提取**，意图分类由三层漏斗接管。

三层漏斗入口 `route_intent_with_trace()`（`intent_router.py`）：

| 层 | 函数 | 输入 | 输出 | 短路条件 |
|---|------|------|------|---------|
| Tier 0 | `_regex_intent_with_confidence()` | 规范化文本 | `(intent, confidence)` | `confidence ≥ 0.80` |
| Tier 1 | `IntentEmbeddingIndex.search()` | 用户查询 | `list[IntentCandidate]` | `top-1.score ≥ 0.75` |
| Tier 2 | `_parse_intent_llm_decision()` | LLM 响应 | `IntentDecision` | `confidence ≥ 0.60` |
...
|------|-------------|-------------|
| 入口函数 | `route_skill_names_with_trace()`（`router.py`） | `route_intent_with_trace()`（`intent_router.py`） |
| 路由层级 | 3 层（正则 → 语义 → LLM） | 3 层（正则 → 语义 → LLM）← **与 Single-agent 一致** |
| 是否调用 LLM 做路由 | 仅在 Tier 2 | 仅在 Tier 2（可选，可配置关闭） |
| 是否使用 Embedding | Tier 1（BGE-M3） | Tier 1（BGE-M3，**复用同一 provider**） |
| 向量化内容 | Skill 元信息（name + description + triggers） | 意图示例语句（INTENT_UTTERANCES 质心向量） |
| 输出粒度 | 精确到具体 Skill（7 个） | 粗粒度意图类别（5 种）+ 置信度 + source trace |
| 是否支持多选 | 正则层支持多 Skill 组合 | 支持 primary + secondary intents（LLM 层） |
| 图结构 | 循环图（ReAct agent ⇄ tools） | 线性 DAG（单次流式处理） |
| 适用场景 | 通用 assistant | APM 可观测性专项 |
| 新 Skill/意图扩展 | 需更新正则规则 + 语义索引 | 更新 INTENT_UTTERANCES + 正则规则 |
| 为什么会这样设计 | 通用场景下 Skill 类型差异大，需要多层策略保证精度 | 复用 single-agent 三层漏斗基建；意图示例语句嵌入 BGE-M3 实现语义分类；正则保留对 APM 术语的精确匹配；LLM 处理混合意图和否定条件句 |
...
| 文件 | 职责 |
|------|------|
| `backend/src/personal_assistant/agent/intent_router.py` | 意图路由核心：`INTENT_UTTERANCES`、`IntentSlots`、`IntentEmbeddingIndex`、`IntentDecision`、`route_intent_with_trace()` |
| `backend/src/personal_assistant/agent/multi_agent.py` | `compile_multi_agent()` 接收 `intent_index`/`intent_llm`，`rewrite_intent` 节点编排三层漏斗 |
| `backend/src/personal_assistant/agent/harness.py` | `_compile_multi_agent()` 构建 `IntentEmbeddingIndex` 和意图分类 LLM 并注入 |
| `backend/src/personal_assistant/config.py` | 新增 6 个 `MULTI_AGENT_INTENT_*` 环境变量配置项 |
| `backend/tests/test_intent_router.py` | 34 个测试覆盖三层漏斗全部路径 |

### 5.9 配置项速查

```bash
# Tier 0: 正则置信度阈值（≥此值短路，不进语义层）
MULTI_AGENT_INTENT_REGEX_THRESHOLD=0.80

# Tier 1: 语义路由（复用 SKILL_ROUTING_* 的 BGE-M3 配置）
MULTI_AGENT_INTENT_SEMANTIC_ENABLED=true
MULTI_AGENT_INTENT_SEMANTIC_THRESHOLD=0.75

# Tier 2: LLM 分类器
MULTI_AGENT_INTENT_LLM_ENABLED=true
MULTI_AGENT_INTENT_LLM_THRESHOLD=0.60
MULTI_AGENT_INTENT_LLM_MODEL=     # 不设则用主 LLM
```

### 5.10 与 Single-agent 的基建复用关系

| 组件 | Single-agent 使用 | Multi-agent 复用 |
|------|------------------|-----------------|
| `OllamaBgeM3EmbeddingProvider` | `router.py` — 嵌入 skill 元信息 | `intent_router.py` — 嵌入意图示例语句（**同一实例**） |
| `OllamaBgeM3Reranker` | `router.py` — 重排 skill 候选 | 可复用（当前未启用，保留扩展点） |
| Qdrant HTTP 客户端模式 | `QdrantSkillVectorIndex` | 可复用（当前用 InMemory，Qdrant 版后续扩展） |
| LLM Judge 模式 | `_llm_route_with_trace()` — 判定 skill | `_parse_intent_llm_decision()` — 判定 intent |
| 三层漏斗编排 | `route_skill_names_with_trace()` | `route_intent_with_trace()` — **同一漏斗模式** |
| 配置前缀 | `SKILL_ROUTING_*` | `MULTI_AGENT_INTENT_*`（新增，embedding 仍用 `SKILL_ROUTING_*`） |
